...
# Doenloads the zonzorp.tgz, if succsesful unzips the .tgz file and extracts the file into the pics folder if that is succsesful it delets the original .tgz file.
wget -q -O ~/public_html/pics/pics.tgz https://zonzorp.net/pics.tgz && tar -xvzf ~/public_html/pics/pics.tgz && rm ~/public_html/pics/pics.tgz
# Task 1: Improve this script to also retrieve and install the files kept in the https://zonzorp.net/pics.tgz tarfile
#     test to make sure the download and extraction commands work
#     then delete the local copy of the tarfile if the extraction was successful

# Make a report on what we have in the Pictures directory
#tests to make sure the .tgz file unziped
test -d ~/pics && test -d ~/public_html/pics && cat <<EOF
