...
# TASK 1: -f Add a test to see if the /etc/resolv.conf file is a regular file
# TASK 2: -h Add a test to see if the /etc/resolv.conf file is a symbolic link
# TASK 3: -d Add a test to see if the /etc/resolv.conf file is a directory
# TASK 4: -r Add a test to see if the /etc/resolv.conf file is readable
# TASK 5: -w Add a test to see if the /etc/resolv.conf file is writable
# TASK 6: -x Add a test to see if the /etc/resolv.conf file is executable
...
test -d /ect/resolv.conf && echo "/ect/resolve.conf is a directory" || echo "ect/resolv.conf is not a directory"
test -r /ect/resolv.comf && echo "/ect/resolve.conf is readable" || echo "ect/resolv.conf is not readable"
test -w /ect/resolv.comf && echo "/ect/resolve.conf is writable" || echo "ect/resolv.conf is not writable"
test -x /ect/resolv.comf && echo "/ect/resolve.conf is executable" || echo "ect/resolv.conf is not executable"


# Tests if /tmp is a directory
# TASK 4: -r Add a test to see if the /tmp directory is readable
# TASK 5: -w Add a test to see if the /tmp directory is writable
# TASK 6: -r Add a test to see if the /tmp directory can be accessed
[ -d /tmp ] && echo "/tmp is a directory" || echo "/tmp is not a directory"
[ -r /tmp ] && echo "/tmp is readable" || echo "/tmp is not readable"
[ -w /tmp ] && echo "/tmp is writable" || echo "/tmp is not a directory"
[ -r /tmp ] && echo "/tmp can be accesed" || echo "/tmp cannot be accessed"
...
read firstNumber
echo 'give me your seccond number'
read secondNumber
...
then
  ...
  echo "your first number is odd"
fi
if [ $((secondNumber%2)) -eq 0 ]
then
  echo "your seccond number is even"
else
  echo "your seccond number is odd"
fi
#if [ $((firstNumber))] multiple
...
[ -v USER ] && echo "The variable USER exists it contains $USER" || echo "The varuable USER exists but is empty"
